...
| B.3 | 2026-07 | **HSE crystal selected and verified** — Abracon ABM8, 16.000 MHz, **CL 8 pF**, ESR ≤ 70 Ω, C0 ≤ 3 pF, ±30 ppm, −40…+85 °C; `C_X1`/`C_X2` = **6 pF**. The old "8–16 MHz, CL 8–12 pF, ESR ≤ 80 Ω" range permitted startup margins from 6.5× down to 2.1×, so it was never a specification. Two circuit corrections came with it: **`C_X2` moves to the crystal side of `R_X1`** (on the MCU side, `R_X1` costs startup margin and limits no drive at all), and the MCU decoupling count drops from six to **five** — the LQFP-64 has **four** VDD pins, not six, counted off DS12288 Figure 7. Adds a **complete 64-pin connection table** to both schematic definitions (wheel §3.1, dash §8.0), cross-checked against the signal maps and the firmware by `check-consistency.py`. Drive level (117 µW est. vs 100 µW max) and cold start are now explicit bring-up measurements |
