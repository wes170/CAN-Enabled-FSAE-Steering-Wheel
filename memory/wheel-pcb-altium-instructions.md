...
3. Mandatory footprint checks (print datasheet page, tick off): STM32G474RET6 LQFP-64 (0.5 mm pitch — verify pad 0.28×1.5 mm class), TJA1051 SO-8, **PEC09 right-angle** — 5 electrical terminals (A/B/C encoder + D/E switch) **plus locating lug(s)**, not 3; see `wheel-schematic-complete.md` §5.2. (THT — verify the body sits flat against the board and the shaft exits *parallel* to the PCB at the intended edge; check shaft length 15/20/25 mm against the faceplate depth before committing), **PEC11H — ⚠ the bushing is `M7 × 0.75`, per the Bourns drawing in `hardware/lib/`. The Ø9.5 mm figure this line used to carry is NOT supported by that datasheet (defect 8.23) and must be re-derived before the faceplate is machined — an M7 thread needs roughly a 7.1–7.5 mm clearance hole, not 9.5 mm.** Anti-rotation on this part is the **2 × Ø2.2 mm locating posts that land in the PCB**, not a faceplate slot; if a faceplate anti-rotation feature is wanted it has to be confirmed against the mechanical drawing rather than assumed. Both encoders ship with one flat washer and one M7 × 0.75 nut. **PEC09 has a locating lug too** — provide its hole, WS2812B-2020 (2.2×2.0 mm, pin-1 dot orientation), **Sharp LS027B7DH01 display, `J3` 10-pin 0.5 mm ZIF — BOTTOM-side contact, now confirmed** from the panel spec Figure 8-1 (it recommends SMK `CFP-4610-0150F` / Molex `51441-1093`, both bottom-contact — **both now deprecated**, superseded by **Hirose `FH12-10S-0.5SH(55)`, LCSC `C506791`**, verified 10P / 0.5 mm / bottom contact / hinged-lid ZIF / 0.3 mm FPC / −40…+85 °C). Contacts mirror if you pick top-contact — this used to be an open item and is now closed, **Molex 204711-0001 vertical USB-C — 24 pads at 0.50 mm pitch PLUS six through-hole anchor slots** (4 shell + 2 middle blade); check against drawing 2047110001 rev B sheet 3, *not* against the dash's 16-pin part, which is a different connector. KSC4 tactiles, JST-GH horizontal, TC2030 footprint (no part, copper+3 locating holes), **LMR36015 VQFN-HR-12 (RNX)** — 12 pads incl. the large SW/PGND thermal pads; verify against the datasheet package drawing, *not* a generic QFN-12 (defect 5.1 replaced the AP63205 TSOT-26 here), plus its inductor. AP2112K SOT-25.
