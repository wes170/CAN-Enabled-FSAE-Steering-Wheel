...
| 5V rail (~1.6A) | ⚠ **LMR33630 REJECTED — 42 V abs max vs a 53.3 V TVS clamp.** Replacement: a **60 V-class buck ≥2 A** (LMR36520 / TPS54360 class — **selection open, verify before adopting**) | This is now the board's only high-voltage-exposed converter, and it must carry both the 5 V loads (~0.5 A: servo buffers, sensor excitation, CAN) **and** the 3.3 V rail reflected through the AP63203 (~1.0 A), so size for **~1.6 A** — which is why the 1.5 A LMR36015 used on the wheel is too marginal here. The earlier "verify the clamp with a scope" note was correct instinct applied to the wrong part: the arithmetic (`datasheet-verification.md` §5) shows no SMBJ standoff/clamp pair fits between a jump start and a 42 V limit. Raise the converter rating instead. |
